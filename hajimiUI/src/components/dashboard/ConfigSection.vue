...
async function handleSaveAll() {
  if (!sharedPassword.value) {
    overallError.value = 'Operator password required'
  ...
  try {
    ...
    if (successes.length && !errors.length) overallSuccess.value = 'Saved: ' + successes.join('; ')
    else if (successes.length && errors.length) overallSuccess.value = 'Partial save: ' + successes.join('; ')
    await dashboardStore.fetchDashboardData()
  } catch (e) {
    overallError.value = e.message || 'Save failed'
  } finally {
...
<template>
  <section class="section">
    <div class="section__header">
      <div class="section__title">Configuration</div>
      <button
        v-if="!enableVertex"
    ...
    <!-- Vertex mode: only show Vertex + Version -->
      ...
      <div v-else class="card">
        <div class="card__body" style="display:flex;flex-direction:column;gap:var(--sp-5);">
          ...
          <div class="row row--between" style="border-top:1px solid var(--border);padding-top:var(--sp-4);">
            ...
            <div class="row">
              <button class="btn btn--secondary btn--sm" @click="isExpanded = false">Cancel</button>
              <button
                class="btn btn--primary btn--sm"
                @click="handleSaveAll"
                :disabled="isOverallSaving"
              >
                {{ isOverallSaving ? 'Saving…' : 'Save all' }}
              </button>
            </div>
          </div>
        </div>
