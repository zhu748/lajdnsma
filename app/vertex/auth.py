...
def validate_settings():
    ...
    if google_credentials_json:
        try:
            ...
            vertex_log(
                "error",
                "Google Credentials JSON is not valid JSON. Please check the format.",
            )
            return False
    ...
    if not project_id:
        vertex_log(
            "warning",
            "Vertex AI Project ID is not set. Required for non-API key methods.",
        )
    ...
    if not location:
        vertex_log(
            "warning", "Vertex AI Location is not set, using default: us-central1"
        )
